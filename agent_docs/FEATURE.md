...
## F-03: Local Ambient-Auth Preflight [2026-08-30] [2026-09-01]
- Status: [x] done
...
- Completion note: All 3 ACs addressed. AC-01/AC-02 fully implemented and unit-tested (9 tests, `src/preflight.rs`) — PATH resolution via a plain directory scan (genuinely zero subprocess spawn for that check, not just in spirit), login check via macOS Keychain presence of the `"Claude Code-credentials"` entry (`security find-generic-password -s ...`, existence-only, the credential value is never read or logged). **This corrects an assumption carried in `SPEC.md`/`DICT.md` since Stage 1 scoping**: earlier research (chili-jar's own adapter code) pointed at a `~/.claude/.credentials.json` file as the credential store; checking this machine's actual, currently-logged-in Claude Code install directly (`security find-generic-password -s "Claude Code-credentials"`, `ls ~/.claude/`) showed real credentials live in the macOS Keychain instead, not that file — implemented and documented against the verified mechanism, not the earlier assumption. Both failure branches are unit-tested via a `check_with(bin_name, keychain_service)` parameterization rather than mutating the real process-global `PATH`, consistent with `DICT.md`'s "Testing conventions" lesson from F-01/F-02. AC-03 (recording a preflight failure's distinct category on a `runs` row) is **not yet implementable** — it names a `runs` row, which is F-08's persistence layer, not built yet; `PreflightError` is already a type distinct from F-04's forthcoming subprocess-error type, so F-08 has a clean category to map once it lands. This is a forward dependency in how AC-03 was originally worded, not a scope cut — flagging explicitly per AGENT.md's AC-conflict handling rather than silently marking it complete. Follow-up for F-08: wire `PreflightError` into `runs.exit_reason` and add the AC-03 test there. `preflight` module isn't called from `main.rs` yet — no consumer exists until F-04 — `#[allow(dead_code)]` on the module import documents this as deliberate, to be removed when F-04 wires it in next.

## F-04: Claude Subprocess Runner [2026-08-30] [2026-09-01]
- Status: [x] done
...
- Completion note: All 6 ACs implemented and tested — 20 unit tests in `src/process.rs`: pure-function tests for every JSON-parsing branch (hand-built fixtures, zero subprocess cost), plus spawn-mechanics tests against a fake stand-in script (args, `--resume`, cwd, non-zero exit, missing binary) rather than the real `claude` binary — invoking the real network-calling CLI in an automated suite that gets re-run 5–10× per feature would be slow, flaky on network conditions, and would burn real subscription usage for no correctness benefit the fake-script approach doesn't already give. `cargo build`, `cargo clippy --all-targets`, `cargo fmt --check` clean; full suite (58 tests) re-run 5× with zero flakes. **One real, load-bearing discrepancy found and fixed via a single manual real-CLI check, not by trusting secondhand code**: chili-jar's adapter (which `SPEC.md`/this description explicitly cite as the shape to follow) reads a `cost_usd` field; running the actual `claude` 2.1.252 on this machine once (`claude --print --output-format json "say the single word: pong"`, in `/tmp`, one real call — not repeated, not part of the automated suite) showed the real field is `total_cost_usd`, and that current output is a single flat object rather than an array of events (this parser's array-or-single-object handling already covered that shape correctly; only the field name was wrong). Fixed to read `total_cost_usd` first with `cost_usd` as a fallback for older/other CLI behavior, with a fixture test built from the real captured output (trimmed, nothing sensitive) locking this in. AC-06 (no git/MCP code) verified by inspection — there is none in this module, nothing to test the absence of. No AC conflicts otherwise. No follow-up items for the next CLAUDE-PM pass.

## F-05: Continuation Signal — Prompt Convention & Parsing [2026-08-30] [2026-09-01]
- Status: [x] done
...
- Completion note: All 5 ACs implemented and covered by 16 unit tests (`src/signal.rs`) — pure functions, no subprocess involved. `build_prompt` covered for context/no-context shape (AC-01); `parse_continuation_signal` covered for the full trailer, missing `RECHECK_AFTER` (AC-04, a valid `None` not an error), a missing `NEXT_ACTION` line (AC-02/parse-error path for F-06), an unparseable duration (AC-03), and trailer lines embedded among surrounding prose (realistic case — the agent's response isn't only the trailer). One deliberate leniency beyond the AC's literal text: the label/reason separator accepts a plain ASCII " - " as a fallback to the documented em dash, since LLM output reproducing an exact Unicode character isn't fully guaranteed — the presence of `NEXT_ACTION:` is what AC-02 actually requires, the separator character is a display nicety. AC-05 (no branching on `next_action`'s value) verified by grep across `src/` — the one `match (next_action, reason)` hit is an `Option` presence check ("did we find the line"), not a branch on what the value says, which is the distinction AC-05 actually draws. `cargo build`, `cargo clippy --all-targets`, `cargo fmt --check` clean; full suite (71 tests) re-run 5× with zero flakes. Not yet wired into any CLI command — same as F-03/F-04, its consumer is F-10. No AC conflicts. No follow-up items.

## F-06: Child Crash Handling & Bounded Retry [2026-08-30] [2026-09-01]
- Status: [x] done
- AC: AC-01, AC-02, AC-03
- Ticket:
...
- Completion note: All 3 ACs implemented and covered by 6 unit tests (`src/retry.rs`). The retry policy itself (`run_with_retry_using`) is parameterized on an injectable attempt closure rather than always going through a real or fake subprocess — a `Cell<u32>` counter lets tests deterministically simulate "fails once then succeeds" and "fails both times" without spawning anything, and also proves the retry never fires a third time (`calls.get() == 2` asserted in the both-fail case). `run_with_retry` (the real entry point) composes F-04's `process::run` and F-05's `signal::parse_continuation_signal` behind that same injection point, so production wiring and the test-covered policy are the same code path. `RunOutcome.retried: bool` carries the AC-03 "retry fact available for persistence" forward for F-08 to write into `runs.retry_count`. `cargo build`, `cargo clippy --all-targets`, `cargo fmt --check` clean; full suite (76 tests) re-run 5× with zero flakes. Not yet wired into any CLI command — its consumer is F-10. No AC conflicts. No follow-up items.

## F-07: Local State Store [2026-08-30] [2026-09-01]
- Status: [x] done
- AC: AC-01, AC-02, AC-03, AC-04, AC-05
- Ticket:
...
- Completion note: All 5 ACs implemented and covered by 21 unit tests (`src/store/mod.rs`, `src/store/runs.rs`, `src/store/schedules.rs`), every one against `Connection::open_in_memory()` — no filesystem, no `RUNNER_HOME`, no env-var race class at all for this feature's tests. Both `runs.rs` and `schedules.rs`'s test modules call the real (private but visible to child modules) `migrate` function directly rather than hand-copying schema SQL a second time, so there's no duplicate-schema-drift risk to guard against. Idempotency (AC-01) is achieved via `CREATE TABLE IF NOT EXISTS` plus a `PRAGMA user_version` set defensively for a future migration to check — there's only one schema version so far, so there's no actual version-branching logic yet, just the groundwork; noted explicitly rather than silently claiming more machinery exists than does. `bundled` rusqlite pulls in some wasm-related transitive dependencies in `Cargo.lock` that looked surprising at first glance — confirmed via a clean build that the real native path (`libsqlite3-sys` via `cc`) is what actually compiles for this target, nothing wasm-related is built. `cargo build`, `cargo clippy --all-targets`, `cargo fmt --check` clean; full suite (92 tests) re-run 5× with zero flakes. No AC conflicts. No follow-up items for the next CLAUDE-PM pass.

## F-08: Run Lifecycle Persistence [2026-08-30] [2026-09-01]
- Status: [x] done
- AC: AC-01, AC-02, AC-03, AC-04
- Ticket:
...
- Completion note: All 4 ACs implemented and covered by 5 new unit tests in `src/persist.rs` (81 total unit tests), using the same closure-injection pattern F-06's `retry.rs` established (`run_and_persist_using` takes the attempt as an injected closure, so success/failure paths are tested with canned outcomes, no subprocess involved). **A real gap found while implementing AC-04, not in the original F-07 schema**: reconciling "any running row whose owning process is confirmed not alive" requires knowing *which* process owns a row — the original `runs` schema had no such column, and AC-04's literal wording ("PID file absent/stale") assumed the daemon's single PID file would somehow suffice, which doesn't work once you consider that a concurrently-running `runner status` invocation must be able to tell a legitimately-in-flight daemon-owned run from a genuinely orphaned one, not just treat every `running` row as automatically stale. Added `runs.owner_pid` directly to F-07's `migrate()` `CREATE TABLE` statement (schema version bumped 1→2; no `ALTER TABLE` migration path needed since Stage 1 has never been released — no real `runner.db` exists anywhere with the old shape). `reconcile_interrupted_runs` checks each `running` row's `owner_pid` individually via `pid::process_alive` (F-01's own helper) rather than assuming. Test `reconcile_marks_only_rows_owned_by_dead_processes` proves both halves: a row owned by a confirmed-dead pid gets reconciled, a row owned by the live test process itself does not. `cargo build`, `cargo clippy --all-targets`, `cargo fmt --check` clean; full suite (97 tests) re-run 5× with zero flakes. No follow-up items for the next CLAUDE-PM pass beyond what's already noted in `DICT.md`.
