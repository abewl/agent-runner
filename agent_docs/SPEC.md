...
- AC-06: The chain is bounded by a hard maximum-turn cap, defaulting to 10; reaching the cap stops the chain (logged/reported as hitting the cap, distinguishable from a clean `CHAIN_CONTINUE: no` stop) rather than looping indefinitely.
...
- AC-12: Every `claude` invocation — chained or `--once` — includes `--permission-mode acceptEdits`, so file write/edit operations (the core capability any real task needs) don't block on an unanswerable interactive permission prompt in non-interactive mode. Fixed and always-applied in this batch, not user-configurable via a Runner flag; a target repo can still layer its own `.claude/settings.json` for finer-grained tool permissions, since `claude` already reads settings from its working directory and Runner does not override or conflict with that. Discovered as a real, pre-existing gap in F-04's original invocation (affects `--once` too, not just chains) during F-16 manual verification — fixed here since it was found here, with an addendum on F-04's own completion note.
- AC-13: If the same `next_action` label (not the reason text, and not the full result) repeats for 3 consecutive turns within a chain while `chain_continue` keeps reporting `true`, the chain stops immediately rather than continuing toward the max-turn cap — logged/reported as a stuck-detection stop, distinguishable from both a clean stop (AC-05) and a cap-reached stop (AC-06). The comparison is plain string equality on the label only; Runner does not parse, categorize, or otherwise act on what the label means.
- AC-14: `RECHECK_AFTER` is a mandatory line in every trailer response — never conditionally omitted — with exactly two valid forms: the literal token `NONE`, or a fixed-width duration of exactly two digits followed by exactly one unit letter (`m`/`h`/`d`), e.g. `05m`, `02h`, `01d`. Both `NONE` and a genuinely absent line map to `recheck_after: None`; any other value (including the previously-accepted variable-width forms like `2h`/`1d`) is still a real parse error, not silently defaulted — this closes an ambiguity gap between the prompt's natural-language instruction and the parser's strict grammar, it does not loosen the parser's strictness.
