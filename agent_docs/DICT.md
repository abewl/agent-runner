...
## Non-interactive `claude` blocks on file writes without a permission flag (F-16)

`claude --print` with no permission flag will block on any tool call that needs approval (Write, Edit, some Bash) — there's no TTY to show an interactive prompt through, so it just fails. This isn't specific to chaining; it affects every caller, including a single manual `runner run --once`, and has been true since F-04 shipped the invocation without ever passing a permission flag. Fixed by always including `--permission-mode acceptEdits`. Deliberately **not** `--dangerously-skip-permissions`/`--permission-mode bypassPermissions` — Anthropic's own `claude --help` text recommends those only for sandboxes with no internet access, which describes a CI container, not the user's own real, networked development machine Runner actually runs on. A target repo can still layer its own `.claude/settings.json` for finer-grained tool permissions — `claude` already reads settings from its working directory, and Runner's invocation doesn't override or conflict with that, so there was no need to build a permissions layer of Runner's own.

---

## Harden the shared contract, don't just soften the parser (F-16)

A real bug, found via manual verification, not by inspection: the original `RECHECK_AFTER` trailer wording said "omit this line if it doesn't apply" — asking the model to make a *structural* decision (delete an entire line) based on its own judgment, rather than choose from a fixed set of tokens it was actually given. In practice the model sometimes wrote a placeholder ("none") instead of omitting the line, and the old variable-width parser (any digits + one unit letter) treated that as a fatal parse error for the *whole* signal, not just that field — cheap for one manual run, chain-ending under F-16's "a hard failure on any turn aborts the entire chain."

The fix that matters here is not "make the parser more forgiving of garbage" — that just hides the mismatch instead of closing it, and an even sloppier future response could still slip through. The actual fix is giving both sides of the boundary (the free-text prompt instruction and the strict Rust parser) the *same* closed, explicit vocabulary: `RECHECK_AFTER` is now a mandatory line with exactly two valid shapes — the literal token `NONE`, or a fixed two-digit-plus-unit-letter duration (`05m`/`02h`/`01d`, never a variable-width `2h`/`30m`-style number). Anything outside those two shapes is still a real parse error. **The general lesson for any future trailer field**: never phrase an instruction as "include X unless it doesn't apply, in which case omit it" — that's a formatting decision layered on top of the actual value, and formatting decisions are exactly where a strict parser and a free-text generator drift apart. Always give a fixed, explicit token for "doesn't apply" instead, and always require the line.

---

## Return an explicit reason type, don't make the caller infer it from data shape (F-16)

`persist::run_chain`'s loop can stop for three different reasons — a clean `CHAIN_CONTINUE: no`, a repeated-`next_action` stuck-detection stop, or hitting the max-turn cap. The first version of this returned only `Vec<RunOutcome>` and expected the caller to infer which reason applied by checking `outcomes.len() == max_turns` and the last outcome's `chain_continue` flag. That inference is wrong in a real, findable case: if the stuck-detector happens to trigger on exactly the last allowed turn (e.g. `max_turns` chosen close to the stuck threshold), the shape looks identical to "hit the cap," and the two get silently conflated — caught while writing a dedicated test for the stuck-before-cap case, not by inspection. Fixed by returning an explicit `ChainStopReason` enum (`AgentDone` / `Stuck` / `MaxTurnsReached`) alongside the outcomes, so the reason is a fact the function asserts, not a fact the caller reconstructs from a data shape that more than one path can produce.
