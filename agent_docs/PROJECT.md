...
- **CLI** — the primary manual-control surface: `runner daemon start|stop|status`, `runner run <task>`, `runner status` / `runner ps`, `runner logs <run-id>`, `runner cron add|list|remove`.
- **TUI** — a simple, read-only status dashboard (run list with live status, per-run detail/result view), reading the same local state store the CLI reads. This is not Herdr's terminal multiplexer — it never renders the `claude` subprocess's raw terminal output, because there is none to render (non-interactive `--print` mode); it's a structured status viewer over the run/schedule store.
...
runner (single binary)
  ...
  │                               ── runner repo set <path>
  │                               ── runner run <task>
  │                               ── runner status / ps / logs <id>
