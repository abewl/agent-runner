...
- **PM/Engineer chaining via a self-reported continuation signal, not Runner-side orchestration.** Every prompt Runner sends is one fixed, generic template — Runner never branches on project content or decides whether a turn should scope work (PM) or implement it (Engineer); that judgment stays entirely inside the `claude` call, per `AGENT.md`'s own already-documented protocol. What Runner adds is memory, not judgment: every invocation is instructed to end its response with a fixed trailer —
  ...
  RECHECK_AFTER: NONE, or a fixed two-digit-plus-unit duration, e.g. "05m"/"02h"/"01d"
  CHAIN_CONTINUE: yes|no
  ```
  `RECHECK_AFTER` is a mandatory line with a closed, fixed-width vocabulary — never a line the model conditionally omits — specifically because an earlier "omit it if it doesn't apply" wording asked the model to make a structural decision instead of picking from a fixed set of tokens, and it sometimes filled in a placeholder instead, which a strict variable-width parser treated as a fatal error for the whole signal. `CHAIN_CONTINUE` is E-06/F-16's addition; see below.
  — which Runner parses as a fixed field, the same way it already parses `ready`/`done`/`error` signal lines elsewhere in this design, never as project-specific prose. `next_action`/`reason` are opaque to Runner: stored, displayed (`status`/`logs`/TUI), and handed back as context text on the next invocation of the same task identity — never branched on in code. Two fields Runner's control flow *does* mechanically act on, both content-blind (compared against a fixed value or "now," never interpreted): `recheck_after` (see the cron interaction rule below) and, as of E-06/F-16, `chain_continue` (see "Self-chaining" below).
- **Self-chaining a single task across multiple turns (E-06/F-16) — still not orchestration, a scope refinement of the rule above, not a reversal of it.** Stage 1 originally treated "one `runner run` invocation" and "one `claude` turn" as the same unit — that boundary was slightly wrong: a human-triggered task and a single turn aren't actually the same thing, and forcing a human to manually re-invoke `runner run` after every turn just to let a PM→Engineer→PM loop continue was friction the design didn't need to impose. `runner run <task>` now chains by default: after each turn, unless the response's `CHAIN_CONTINUE: yes|no` trailer line says `yes`, Runner resumes the same session and fires the next turn immediately, inside one synchronous CLI invocation — no daemon involved. `--once` opts back into the original single-turn behavior. This is deliberately a **different, new trailer field** from `recheck_after`, not a repurposing of it — `recheck_after` keeps meaning exactly what it always meant (a future cron-recheck hint); conflating "check again later" with "keep going right now" would have been two concerns wearing one field. A hard iteration cap (10 by default) bounds every chain (a resource guard, the same category as F-06's bounded retry — not a decision about what the agent should do), and a hard failure on any single turn aborts the whole chain rather than silently continuing. A stuck-detector stops a chain early, before the cap, if the same `next_action` label repeats 3 turns straight — a plain string-equality check on the label, never an interpretation of what it means, same category of mechanical check as everything else here. Cron's own cadence and recheck-gating rules below are completely unaffected — this only changes what a single, human-triggered `runner run` does.
- **Every claude invocation needs `--permission-mode acceptEdits` to do real work headlessly — a pre-existing gap, not new to this epic.** Discovered during E-06/F-16's own manual verification: non-interactive `claude` blocks on any file write/edit by default, with no way to answer a permission prompt that has no TTY to show through. This affected `runner run` since F-04 shipped, chaining or not — fixed as part of this epic since it was found here. Fixed, not user-configurable in this batch; a target repo can still layer its own `.claude/settings.json` for finer-grained tool permissions, since `claude` already reads settings from its working directory. `--dangerously-skip-permissions`/`bypassPermissions` were considered and rejected — Anthropic's own docs recommend those only for sandboxes with no internet access, which describes a CI container, not a user's real development machine.
